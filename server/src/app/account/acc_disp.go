...
func regProp() {
	network.Instance.RegProto(proto.CodeCSAccountReg, csAccountReg)
	network.Instance.RegProto(proto.CodeCSAccountLogin, csAccountLogin)
}
...
func sendAccountInfo(sender app.ISender, account *Account) {
	...
	infos := make([]*proto.SCAccountInfo_Roleinfo, len(account.Roles))
	for i, info := range account.Roles {
		infos[i] = &proto.SCAccountInfo_Roleinfo{
			Id: info.Id,
			Name: info.Name,
			Mtime: info.LogoutTime.Unix(),
...
func csAccountReg(ses network.Session, data interface{}) {
